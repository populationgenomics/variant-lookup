...
- **Retriable errors** are signalled by codes ending in `_UPSTREAM_TIMEOUT`: `NORMALIZATION_UPSTREAM_TIMEOUT`, `BACK_TRANSLATE_UPSTREAM_TIMEOUT`, `RSID_UPSTREAM_TIMEOUT`, `VV_UPSTREAM_TIMEOUT`. These come from `httpx.TimeoutException` on an upstream call (mutalyzer / VV / NCBI) and are transient — the first request for a novel accession pays the cold-fetch cost, so a retry after the cache has warmed usually succeeds. Callers should re-issue these (with backoff) before recording them as failures. All other `error.code`s are non-retriable: `VARIANT_CLEANUP_FAILED`, `NO_GENOMIC_COORDS` (VV returned but has no GRCh38 mapping), the mutalyzer input-error codes surfaced from upstream's structured 422 body (`NORMALIZATION_EINTRONIC`, `NORMALIZATION_ESEQUENCEMISMATCH`, `NORMALIZATION_ERETR`, etc.), and `*_UPSTREAM_ERROR` (transport / non-timeout HTTP error).
